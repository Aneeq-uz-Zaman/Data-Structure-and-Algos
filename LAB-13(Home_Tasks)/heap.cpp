...
public:
    ...
    static bool isHeap(vector<int> &arr, bool isMinHeap) {
        for (int i = 0; i < arr.size(); i++) {
            ...
            int r = 2 * i + 2;
            if (l < arr.size()) {
                if (isMinHeap && arr[i] > arr[l]) return false;
                if (!isMinHeap && arr[i] < arr[l]) return false;
            }
            if (r < arr.size()) {
                if (isMinHeap && arr[i] > arr[r]) return false;
                if (!isMinHeap && arr[i] < arr[r]) return false;
            }
        }
        return true;
